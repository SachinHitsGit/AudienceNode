...
DB_PATH      = "/home/sachinkumar/Desktop/sponser_proof_gen/Backend/twitch_chat.db"
...
# EXPOSED BACKEND ENTRY POINT FOR FASTAPI

def run_analysis_pipeline(start_time: str, stop_time: str):
    usernames, messages = fetch_messages(DB_PATH, start_time, stop_time)

    if len(messages) < 4:
        return {"error": "Insufficient text data gathered during this window to perform clustering."}

    embeddings = embed_messages(messages)

    coordinates = reduce_with_pca(embeddings, n_components=3)

    best_k = find_optimal_k(coordinates, max_k=MAX_K, plot=False)
...
if __name__ == "__main__":
    ...
    if len(messages) < 4:
        print("Not enough messages to cluster. Add more data first.")
        exit()
