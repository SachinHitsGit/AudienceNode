...
def run_pipeline(
    ...
    print("📥 Fetching and compressing messages...")
    compressed = compress_messages(conn, poll_start, poll_stop)
    print(f"   → {sum(c['weight'] for c in compressed)} raw messages compressed to {len(compressed)} unique strings")

    print("🔍 Running sponsor keyword scan...")
    ...
    output = {
        ...
        "top_messages":    compressed[:20],  # top 20 by weight, for frontend
    }
...
if __name__ == "__main__":
    ...
    # Save to file so FastAPI can serve it later
    with open("poll_result.json", "w") as f:
        json.dump(result, f, indent=2)
    print("\n💾 Saved to poll_result.json")